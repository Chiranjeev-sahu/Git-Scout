    ...
    <div className="relative min-h-screen w-full bg-[#FBFCF8] overflow-hidden font-sans selection:bg-black selection:text-white">
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-muted-foreground">
          <Link to="/dashboard" className="hover:text-black transition-colors">Discover</Link>
          <a href="#" className="hover:text-black transition-colors">Trending</a>
        </div>

        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" className="font-bold">Log In</Button>
          <Button onClick={() => navigate("/dashboard")} size="sm" className="bg-black text-white rounded-xs px-6 hover:bg-zinc-800 transition-all">
            Sign Up
          </Button>
        </div>
      </nav>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center pt-24 px-4">
        ...
        <div className="relative max-w-4xl text-center">
          ...
          >
            Find your next <span className="italic font-medium pr-2">OSS</span> <br />
            <span className="relative inline-block">
              contribution in{" "}
              <LineShadowText className='italic tracking-tighter font-extralight'>Seconds</LineShadowText>
            </span>
          ...
          >
            Stop aimless scrolling. Instantly discover tailored open-source <span className="text-black font-medium">repositories</span> and <span className="text-black font-medium">issues</span> that fit your stack and experience level.
          </motion.p>
          ...
          >
            <ShinyButton onClick={() => navigate("/dashboard")} className="h-12 px-10 bg-black text-white text-md font-bold shadow-2xl shadow-black/20 hover:scale-[1.02] active:scale-[0.98] transition-all">
              Start Scouting
            </ShinyButton>

            <InteractiveHoverButton onClick={() => navigate("/dashboard")}>
              View Trending
